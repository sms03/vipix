...
const PixelBlast: React.FC<PixelBlastProps> = ({
  ...
  renderer.setPixelRatio(Math.min(window.devicePixelRatio || 1, 1.5));
      ...
      // Add global listeners so interaction works even when other elements overlay the canvas
      const globalPointerDown = (e: PointerEvent) => {
        // Always register a ripple/click
        onPointerDown(e);
      };
      let lastMoveFrame = 0;
      const globalPointerMove = (e: PointerEvent) => {
        ...
        if (now - lastMoveFrame < 16) return; // ~60fps throttle
        lastMoveFrame = now;
        onPointerMove(e);
      };
